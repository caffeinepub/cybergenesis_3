...
const DEBUG_MODE =
  import.meta.env.VITE_DEBUG_3D === "true" ||
...
export const debugGPUContext = (gl: any) => {
  ...
  try {
    ...
    console.error(
      "[GPU Diagnostics] Failed to retrieve GPU information:",
      error,
    );
  }
};
...
export const detectOverlappingRenderers = () => {
  ...
  if (canvases.length > 1) {
    console.warn(
      "[Renderer Detection] Multiple canvas elements detected - potential context conflict",
    );
  }
  ...
  console.log(
    "[Renderer Detection] No external THREE.Scene or WebGLRenderer detected - unified context confirmed",
  );
};
...
export const testSceneStability = () => {
  ...
  if (canvas) {
    ...
    console.log(
      "[Scene Stability Test] Canvas dimensions:",
      rect.width,
      "x",
      rect.height,
    );
    ...
    if (rect.width === 0 || rect.height === 0) {
      console.error(
        "[Scene Stability Test] Canvas dimensions are ZERO - scene will not render",
      );
    }
  } else {
