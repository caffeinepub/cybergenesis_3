...
export default function CanisterDiagnostics() {
  ...
  const [landReport, setLandReport] = useState<CanisterHealthReport | null>(
    null,
  );
  const [assetReport, setAssetReport] = useState<CanisterHealthReport | null>(
    null,
  );
  ...
  const generateRecommendations = (
    ...
    if (overallStatus === "offline") {
      recommendations.push(
        `All gateways failed for ${canisterName}. Check if canister is deployed and running.`,
      );
      recommendations.push(
        "Verify canister ID is correct and matches deployed backend.",
      );
      recommendations.push(
        "Check Internet Computer network status at https://status.internetcomputer.org",
      );
      ...
      const failedGateways = results
        ...
        .map((r) => r.gateway);
      recommendations.push(
        `Some gateways are unstable: ${failedGateways.join(", ")}`,
      );
      ...
      if (successfulGateways.length > 0) {
        ...
        recommendations.push(
          `Use ${fastest.gateway} as primary gateway (${fastest.responseTime}ms response time).`,
        );
      }
    } else {
      ...
      if (slowest.responseTime > 2000) {
        recommendations.push(
          `${slowest.gateway} is slow (${slowest.responseTime}ms). Consider using faster alternatives.`,
        );
      }

      recommendations.push(
        `Optimal gateway: ${fastest.gateway} (${fastest.responseTime}ms response time).`,
      );

      if (fastest.responseTime < 500) {
        recommendations.push(
          `Excellent connectivity to ${canisterName}. All systems operational.`,
        );
      }
    }
  ...
  const runHealthCheck = async (silent = false) => {
    ...
    if (!silent) {
      toast.info(
        "🔄 Starting comprehensive network health check with cache-busting...",
      );
    }
    ...
    try {
      ...
      const landResults = await Promise.all(
        GATEWAYS.map((gateway) =>
          testGatewayConnectivity(gateway, LAND_CANISTER_ID),
        ),
      );

      const landStatus = determineOverallStatus(landResults);
      const landRecommendations = generateRecommendations(
        "LandCanister",
        landResults,
        landStatus,
      );
      ...
      console.log(
        "🔍 Testing AssetCanister connectivity with cache-busting...",
      );
      const assetResults = await Promise.all(
        GATEWAYS.map((gateway) =>
          testGatewayConnectivity(gateway, ASSET_CANISTER_ID),
        ),
      );

      const assetStatus = determineOverallStatus(assetResults);
      const assetRecommendations = generateRecommendations(
        "AssetCanister",
        assetResults,
        assetStatus,
      );
      ...
      if (!silent) {
        ...
        if (allHealthy) {
          toast.success(
            "✅ Network health check completed - All systems HEALTHY",
          );
        } else {
          toast.warning(
            "⚠️ Network health check completed - Some issues detected",
          );
        }
      }
    } catch (error: any) {
      ...
      <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="space-y-2">
            ...
            <div className="space-y-2">
              {report.gatewayResults.map((result) => (
                <div
                  key={result.gateway}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-2">
                {report.recommendations.map((rec, index) => (
                  <div
                    // biome-ignore lint/suspicious/noArrayIndexKey: recommendations don't have unique ids
                    key={index}
      ...
      <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-sm">
        <CardHeader>
          ...
          <CardDescription className="text-gray-400">
            Real-time connectivity testing with cache-busting across all
            Internet Computer gateways
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
          <div className="flex items-center justify-between">
            ...
            <div className="p-4 rounded-lg bg-blue-500/10 border border-blue-500/30 text-sm text-gray-300">
              <div className="flex items-center gap-2 mb-2">
                ...
                <span className="font-semibold">
                  Testing in progress with cache-busting...
                </span>
              </div>
              <ul className="list-disc list-inside space-y-1 text-xs text-gray-400 ml-6">
                <li>Clearing all cached queries for fresh state</li>
                <li>
                  Testing connectivity to LandCanister ({LAND_CANISTER_ID})
                </li>
                <li>
                  Testing connectivity to AssetCanister ({ASSET_CANISTER_ID})
                </li>
        ...
        <Card className="bg-black/40 border-green-500/30 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4">
              <div className="p-4 rounded-lg bg-black/30 border border-gray-700/50">
                ...
                <div
                  className={`text-2xl font-bold ${
                    landReport.overallStatus === "online"
                      ? "text-green-400"
                      : landReport.overallStatus === "degraded"
                        ? "text-yellow-400"
                        : "text-red-400"
                  }`}
                >
                  {landReport.overallStatus === "online"
                    ? "HEALTHY"
                    : landReport.overallStatus.toUpperCase()}
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {
                    landReport.gatewayResults.filter(
                      (r) => r.status === "success",
                    ).length
                  }
              ...
              <div className="p-4 rounded-lg bg-black/30 border border-gray-700/50">
                <div className="text-sm text-gray-400 mb-1">
                  AssetCanister Status
                </div>
                <div
                  className={`text-2xl font-bold ${
                    assetReport.overallStatus === "online"
                      ? "text-green-400"
                      : assetReport.overallStatus === "degraded"
                        ? "text-yellow-400"
                        : "text-red-400"
                  }`}
                >
                  {assetReport.overallStatus === "online"
                    ? "HEALTHY"
                    : assetReport.overallStatus.toUpperCase()}
                </div>
                <div className="text-xs text-gray-500 mt-1">
                  {
                    assetReport.gatewayResults.filter(
                      (r) => r.status === "success",
                    ).length
                  }
