...
export default function NetworkDiagnostics() {
  ...
  const performHealthCheck = useCallback(async (): Promise<{
    ...
    if (!assetActor) return { success: false, latency: 0 };
    ...
    try {
      ...
      console.log(
        `[Network Diagnostics] Health check successful - Latency: ${latency}ms`,
      );
  ...
  const getLatencyStatus = () => {
    ...
    if (metrics.latency < 100)
      return { color: "text-[#00ff41]", label: "EXCELLENT" };
    if (metrics.latency < 300)
      return { color: "text-[#00d4ff]", label: "GOOD" };
      ...
      >
        <div className="flex items-center space-x-3">
          ...
          <div className="text-left">
            ...
            <p className={`text-xs font-mono ${statusDisplay.color}`}>
              {statusDisplay.label}
            </p>
          </div>
        </div>
        <div className="flex items-center space-x-4">
          {metrics.latency !== null && (
            <div className="text-right">
              ...
              <p
                className={`text-sm font-mono font-bold ${latencyStatus.color}`}
              >
                {metrics.latency}ms
              </p>
            </div>
          ...
          <Activity
            className={`w-4 h-4 text-[#00d4ff] transition-transform ${isExpanded ? "rotate-180" : ""}`}
          />
        </div>
      </button>
      ...
      {isExpanded && (
        <div className="px-4 pb-4 space-y-4 border-t border-[#00d4ff]/20">
          <div className="grid grid-cols-2 gap-4 mt-4">
            <div
              className={`p-3 rounded-lg border ${statusDisplay.borderColor} ${statusDisplay.bgColor}`}
            >
              ...
              <p
                className={`text-2xl font-bold font-mono ${latencyStatus.color}`}
              >
            ...
            <div className="flex items-start space-x-2 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-yellow-400 mt-1">
                  {metrics.consecutiveFailures} consecutive failure
                  {metrics.consecutiveFailures > 1 ? "s" : ""}
                </p>
              </div>
            </div>
          )}
          {envValidation && (
            <div className="space-y-2">
                ...
                <div className="space-y-1 text-xs font-mono">
                  <div className="flex justify-between items-center p-2 bg-[#00ff41]/5 rounded border border-[#00ff41]/20">
                    ...
                    <span className="text-[#00ff41] font-bold">
                      {envValidation.config.network.toUpperCase()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-gray-500">Canister ID:</span>
                    <span className="text-[#00d4ff]">
                      {envValidation.config.assetCanisterId.slice(0, 15)}...
                    </span>
                  </div>
                  <div className="space-y-1">
                    <span className="text-gray-500">Gateway Nodes:</span>
                    {envValidation.config.gateways.map(
                      (gateway: string, idx: number) => (
                        <div
                          key={gateway}
                        ...
                        >
                          ...
                          {idx === 0 && (
                            <span className="text-[#00ff41] text-[10px] px-1 py-0.5 bg-[#00ff41]/10 rounded">
                              PRIMARY
                            </span>
                          )}
                        </div>
              ...
              {!envValidation.isValid && (
                <div className="space-y-1">
                  {envValidation.errors.map((error: string) => (
          ...
          {envValidation?.isValid && envValidation.config?.network === "ic" && (
            <div className="p-3 bg-[#00ff41]/10 border border-[#00ff41]/30 rounded-lg">
              ...
              <p className="text-xs text-[#00ff41]/70 mt-1 ml-6">
                All operations are targeting the mainnet (ic) network with
