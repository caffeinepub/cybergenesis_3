...
export default function AssetCanisterDiagnostics() {
  ...
  const assetActorTyped = assetActor as any;
  ...
  const addLog = (level: "info" | "warning" | "error", message: string) => {
    ...
    console.log(
      `[AssetCanister Diagnostics] [${level.toUpperCase()}] ${message}`,
    );
  };
  ...
  const runComprehensiveDiagnostics = async () => {
    ...
    addLog(
      "info",
      "🔄 Starting comprehensive Asset Canister diagnostics with cache-busting...",
    );
    ...
    try {
      ...
      if (!assetActor) {
        updateTest(
          ...
          performance.now() - startTime1,
        );
        addLog(
          "error",
          `❌ Asset actor is not initialized. Connection status: ${connectionStatus}`,
        );
        ...
        updateTest(
          "Actor Initialization",
          "passed",
          "Actor successfully initialized",
          performance.now() - startTime1,
        );
      ...
      addLog(
        "info",
        "Test 2/8: Testing network connectivity with cache-busting...",
      );
      ...
      try {
        ...
        const response = await fetch(
          `https://ic0.app/api/v2/canister/${ASSET_CANISTER_ID}/query?${cacheBuster}`,
          {
            method: "POST",
            headers: {
        ...
        if (response.ok) {
          updateTest(
            ...
            "passed",
            `HTTP ${response.status} - ${duration2.toFixed(0)}ms`,
            duration2,
          );
          addLog(
            "info",
            `✅ Network connectivity confirmed (${duration2.toFixed(0)}ms)`,
          );
        } else {
          updateTest(
            "Network Connectivity",
      ...
      if (assetActor) {
        try {
          const role = await assetActorTyped.getCallerUserRole();
      ...
      if (assetActor) {
        try {
          const assets = await assetActorTyped.listAssets();
      ...
      if (assetActor) {
        try {
          const isAdmin = await assetActorTyped.isCallerAdmin();
        ...
        addLog(
          "error",
          "❌ Cannot check admin permissions - actor not available",
        );
      }
      ...
      if (landActor) {
        ...
        addLog(
          "warning",
          "⚠️ Cannot query cycle balance - land actor not available",
        );
      }
      ...
      if (assetActor) {
        try {
          const glbModels = await assetActorTyped.listGLBModels();
          const duration7 = performance.now() - startTime7;
          updateTest(
            "GLB Models Inventory",
            "passed",
            `${glbModels.length} GLB models`,
            duration7,
          );
          addLog("info", `✅ GLB models inventory: ${glbModels.length} models`);

          if (glbModels.length > 0) {
            for (const [filename, url] of glbModels as [string, string][]) {
      ...
      if (landActor && assetActor) {
        try {
          ...
          await assetActorTyped.getCallerUserRole();
          const duration8 = performance.now() - startTime8;
          updateTest(
            "Inter-Canister Communication",
            "passed",
            "Both canisters responsive",
            duration8,
          );
          ...
          addLog(
            "error",
            `❌ Inter-canister communication failed: ${error.message}`,
          );
        }
      } else {
        updateTest(
          "Inter-Canister Communication",
          "failed",
          "One or both actors not available",
        );
        addLog(
          "error",
          "❌ Cannot test inter-canister communication - actors not available",
        );
      }
      ...
      addLog(
        "info",
        "🔄 Invalidating all queries to force fresh data fetch...",
      );
      ...
      const passedCount = diagnosticTests.filter((t) =>
        tests.find((test) => test.name === t.name && test.status === "passed"),
      ).length;
      const totalCount = diagnosticTests.length;

      if (passedCount === totalCount) {
        toast.success(
          `✅ Asset Canister diagnostics completed - All ${totalCount} tests PASSED`,
        );
      } else {
        toast.warning(
          `⚠️ Asset Canister diagnostics completed - ${passedCount}/${totalCount} tests passed`,
        );
      }
    } catch (error: any) {
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription className="text-gray-400 mt-1">
                Comprehensive health check with cache-busting and error
                detection
              </CardDescription>
            </div>
            <Button
        ...
        <Card className="bg-black/40 border-cyan-500/30 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="max-h-96 overflow-y-auto space-y-1 font-mono text-xs">
              {logs.map((log, index) => (
                <div
                  // biome-ignore lint/suspicious/noArrayIndexKey: log items don't have unique ids
                  key={index}
        ...
        <Card className="bg-black/40 border-yellow-500/30 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="space-y-2 text-sm text-gray-300">
              {tests.find(
                (t) =>
                  t.name === "Actor Initialization" && t.status === "failed",
              ) && (
                <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                  <strong>Actor Initialization Failed:</strong> Check that
                  VITE_ASSET_CANISTER_ID is correctly configured in .env file.
                </div>
              )}
              {tests.find(
                (t) =>
                  t.name === "Network Connectivity" && t.status === "failed",
              ) && (
                <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                  <strong>Network Connectivity Failed:</strong> Verify internet
                  connection and IC network status at
                  https://status.internetcomputer.org
                </div>
              )}
              {tests.find(
                (t) =>
                  t.name === "Authentication Status" && t.status === "failed",
              ) && (
                <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                  <strong>Authentication Failed:</strong> Ensure you are logged
                  in with Internet Identity.
                </div>
              )}
              {tests.find(
                (t) =>
                  t.name === "Cycle Balance Query" && t.status === "failed",
              ) && (
                <div className="p-3 rounded-lg bg-yellow-500/10 border border-yellow-500/30">
                  <strong>Low Cycles:</strong> Asset Canister may need more
                  cycles. Consider topping up cycles.
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card className="bg-black/40 border-green-500/30 backdrop-blur-sm">
          ...
          <CardContent>
            <div className="p-4 rounded-lg bg-green-500/10 border border-green-500/30 text-sm text-gray-300">
              <strong>✅ Asset Canister is HEALTHY:</strong> All {totalTests}{" "}
              diagnostic tests passed successfully. The canister is fully
