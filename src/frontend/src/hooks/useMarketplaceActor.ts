...
async function selectOptimalGateway(
  ...
  for (const gateway of gateways) {
    try {
      ...
      console.warn(
        `[Marketplace] Gateway ${gateway} unavailable, trying next...`,
      );
    }
  }
    ...
    const initActorWithRetry = async () => {
      ...
      for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
          ...
          console.log("[Marketplace Actor] Config:", {
          ...
          const timeoutPromise = new Promise<never>((_, reject) => {
            setTimeout(
              () => reject(new Error("Marketplace actor timeout (120s)")),
              ACTOR_TIMEOUT,
            );
          });
          ...
          console.error(
            `[Marketplace Actor] Attempt ${attempt + 1} failed:`,
            errorMessage,
          );
