...
export function useAssetUpload() {
  ...
  const uploadGLB = useCallback(
    async (file: File, landTypeName: string): Promise<UploadResult> => {
      ...
      try {
        ...
        if (!window.ic?.plug) {
          throw new Error(
            "Plug wallet not found. Please install Plug wallet extension.",
          );
        }
        ...
        if (!isConnected) {
          throw new Error(
            "Plug wallet not connected. Please connect your wallet first.",
          );
        }
